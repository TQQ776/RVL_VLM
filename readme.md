...
原来的 Franka Cartesian velocity 直控模式仍然保留：
...
~/TQQ_ws/tqq/src/franka_vr_teleop/config/quest3_servo_teleop.yaml
  Servo 模式下 Quest 输入增益和速度上限

~/TQQ_ws/tqq/src/franka_vr_teleop/config/fr3_moveit_servo.yaml
  MoveIt Servo 的平滑、碰撞检查、奇异点和输出话题参数
```
...
```text
按 r 开始录音。
按 q 停止录音，然后 Qwen-Omni 直接理解语音并执行。
按 t 弹出文本对话窗口，输入文本后按 Ctrl+Enter 发送，同样调用 Qwen-Omni。
```
